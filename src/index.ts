...
type RNOLAuthViewModelNSAttributedString = {
...
type RNOLAuthViewModelUIFontDescriptor = {
  textStyle: String
  size: number
}

type RNOLAuthViewModelUIFont = {
  name: string;
  size: number;
  fontAttributes?: RNOLAuthViewModelUIFontDescriptor
}

type RNOLPrivacyTermItem =  {
  title: string
  /**
   * 带协议的URL
   */
 ...
 * @returns
 */
export function requestTokenWithViewController(viewModel?: {
/**
     状态栏样式。 默认 `UIStatusBarStyleDefault`。
     */
     statusBarStyle?: "UIStatusBarStyle.darkContent" | "UIStatusBarStyle.lightContent" | "UIStatusBarStyle.default"; // UIStatusBarStyle
...
//    @property (nullable, nonatomic, copy) OLAuthViewLifeCycleBlock viewLifeCycleBlock;
  ...
  if (Platform.OS === 'ios') {
    ...
    return ExpoGeetestOneloginModule.requestTokenWithViewController(viewModel, callback ? functionToCallbackId(callback) : undefined);
  }
